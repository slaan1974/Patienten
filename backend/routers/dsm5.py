...
from schemas.dsm5_form import Dsm5FormCreate, Dsm5FormUpdate, Dsm5FormOut
from services.dsm5_service import (
    get_form_by_patient,
    get_form,
    create_form,
    update_form,
...
router = APIRouter(prefix="/api/dsm5", tags=["dsm5"])


@router.get("/{patient_id}", response_model=Dsm5FormOut | None)
...
@router.post("/{patient_id}", response_model=Dsm5FormOut, status_code=201)
...
@router.put("/form/{form_id}", response_model=Dsm5FormOut)
...
async def delete_form_endpoint(
    ...
    ip = request.client.host if request.client else None
    await audit_delete(db, "dsm5_forms", form_id, old_dict, user.id, ip)
...
async def get_all_forms_status(
    ...
    forms_result = await db.execute(select(Dsm5Form))
