...
function statusBadge(item) {
  ...
  if (item.form_status === 'definitief') {
    return { label: 'Definitief', class: 'text-green-600 bg-green-50 ring-green-200' }
  }
  return { label: 'Concept', class: 'text-yellow-600 bg-yellow-50 ring-yellow-200' }
}
</script>

<template>
  <div class="space-y-6">
    <h1 class="text-3xl font-bold text-gray-800">DSM-5 Formulieren</h1>
    ...
    <div class="bg-white p-4 rounded-xl shadow-sm border flex flex-wrap gap-3 items-center">
      <div class="flex gap-2 items-center flex-1 min-w-[200px]">
        ...
        <select
          ...
          class="px-3 py-2 border rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-green-500"
        >
        ...
        <tbody class="divide-y">
          <tr v-for="p in filteredPatients" :key="p.patient_id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-right">
              <button
                @click="router.push(`/patienten/${p.patient_id}/dsm5`)"
